...
/**
 * 1.jsonlink源码剥离只使用报错提示的方法，代码格式需做调整(node eslink报错)
 * 2.语法提示智能化
 * 3.做包装做成工具类函数
 */
/* eslint-disable */
    ...
    parse: function parse (input) {
      ...
      while (true) {
        ...
        if (typeof action === 'undefined' || !action.length || !action[0]) {
          if (!recovering) {
            ...
            if (this.lexer.showPosition) {
              errStr = '第' + (yylineno + 1) + '行解析错误:\n' + this.lexer.showPosition() + '\n期望是 ' + expected.join(', ') + ", 得到 '" + this.terminals_[symbol] + "'"
            } else {
              errStr = '第' + (yylineno + 1) + '行解析错误: 期望是 ' +
        ...
        if (action[0] instanceof Array && action.length > 1) {
          throw new Error('解析错误:状态下可能有多个操作: ' + state + ', token: ' + symbol)
        }
